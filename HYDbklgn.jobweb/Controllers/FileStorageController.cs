...
using HYDbklgn.Framework.Model;
using System;
...
namespace HYDbklgn.jobweb.Controllers
{
	...
	{
		private string root = System.Web.Hosting.HostingEnvironment.MapPath(WK.uploadpath.GetAppKeyValue());
		...
		public async Task<object> Upload()
		{
			...
			{
				// Success
				return Request.CreateResponse(HttpStatusCode.OK, provider.FormData["upload_token"]);
			}
			else
			{
				// Fail
		...
		{
			return ResumableConfiguration.Create(identifier: GetId(provider), filename: GetFileName(provider), chunks: GetTotalChunks(provider), GetUploadToken(provider));
		}

		private string GetUploadToken(MultipartFormDataStreamProvider provider)
		{
			var upload_token = provider.FormData["upload_token"];
			return upload_token;
		}
		...
		private void TryAssembleFile(ResumableConfiguration configuration)
		{
			if (AllChunksAreHere(configuration))
			{
				...
				RenameFile(path, Path.Combine(root, configuration.Upload_Token, configuration.FileName));
		...
		private string RenameFile(string sourceName, string targetName)
		{
			...
			string realFileName = Path.Combine(dir, targetName);
